...
### v0.5.0 °™ Discovery Engine (2026-07-23)

**Three-level MemoryCluster system**
- ConflictClusterAdapter °™ adapts v0.4.2 ConflictGraph components into MemoryCluster objects (no re-detection)
- ClaimClusterBuilder °™ groups memories by exact (subject, predicate) pairs via ClaimExtractor
- TopicClusterBuilder °™ TF-IDF + Jaccard similarity clustering with BFS connected components (threshold 0.3)

**Evidence Analyzer**
- EvidenceCollector °™ aggregates governance signals (worthiness, confidence, temporal) per cluster member
- ClusterScoreEngine °™ scores compatibility, dominance, and redundancy within clusters
...
- ConsolidationCandidate °™ typed candidates with action, targets, evidence, expectedGain, riskHint, confidence
- Candidates sorted by confidence descending

**DreamRunner Pipeline**
- DreamRunner °™ orchestrates full Cluster °˙ Evidence °˙ Candidate °˙ Proposal pipeline
- ClusterBuilder °™ combines all 3 levels in a single call
- ProposalMapper °™ translates ConsolidationCandidate to RepairCandidate for RepairQueue

**Architecture boundary: Discovery only, no execution**
- All outputs are proposals °™ never modifies memory
- LLM only explains, never decides
- DreamMode (classic/v0.5/hybrid) for backward compatibility

**Test growth:** 1006 °˙ 1118 (+112), 102 files, 0 regression
