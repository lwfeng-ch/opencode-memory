/**
 * opencode-memory — Semantic Comparator Tests (v0.3.2)
 *
 * Tests the Hybrid-4Layer comparison engine:
 *   - compareText: 3-layer (legacy) or 4-layer (hybrid with embedding) algorithm
 ...
 *
 * v0.3.2 change: compareText/compareMemory are now async (embedding support).
 * Tests without embedding provider use legacy-3layer mode.
 */
...
describe("compareText — 3-layer algorithm (legacy, no embedding)", () => {
  const comparator = new DefaultMemoryComparator()

  test("identical strings return score 1.0", async () => {
    const result = await comparator.compareText("hello world", "hello world")
    expect(result.score).toBe(1.0)
    // No embedding provider → embedding is undefined even for identical strings
    expect(result.breakdown.embedding).toBeUndefined()
  })

  test("completely different strings return score < 0.2", async () => {
    const result = await comparator.compareText("apple banana cherry", "quantum physics theory")
    expect(result.score).toBeLessThan(0.2)
  })

  test("similar but not identical text (YOLOv11 example) returns score > 0.15", async () => {
    const expected = "用户使用 YOLOv11 做风机叶片缺陷检测"
    const actual = "用户正在开发基于 YOLO11 的风机叶片识别项目"
    const result = await comparator.compareText(expected, actual)
  ...
  test("empty strings return score 0", async () => {
    const result = await comparator.compareText("", "")
    expect(result.score).toBe(0)
  })

  test("one empty one non-empty returns score 0", async () => {
    const result = await comparator.compareText("hello world", "")
    expect(result.score).toBe(0)
  })

  test("case insensitivity — same text different case returns 1.0", async () => {
    const result = await comparator.compareText("Hello World", "hello world")
    expect(result.score).toBe(1.0)
  })

  test("word order swap has high similarity > 0.8", async () => {
    const result = await comparator.compareText(
  ...
  test("partial overlap returns score between 0.2 and 0.8", async () => {
    const result = await comparator.compareText(
      "user prefers Python for AI development",
  ...
  test("Chinese text similarity — same meaning different wording", async () => {
    const result = await comparator.compareText(
      "用户是高级Go工程师",
      "用户是一位资深的Go语言开发者",
    )
  ...
  test("returns a number between 0 and 1", async () => {
    const result = await comparator.compareText("some text here", "some other text")
    expect(result.score).toBeGreaterThanOrEqual(0)
    expect(result.score).toBeLessThanOrEqual(1)
  })

  test("TextComparisonResult has breakdown structure", async () => {
    const result = await comparator.compareText("hello world", "hello there")
    expect(result).toHaveProperty("score")
    expect(result).toHaveProperty("breakdown")
    expect(result.breakdown).toHaveProperty("tokenJaccard")
    expect(result.breakdown).toHaveProperty("tfidf")
    expect(result.breakdown).toHaveProperty("bigram")
    // No embedding provider configured → embedding should be undefined
    expect(result.breakdown.embedding).toBeUndefined()
  })
})
...
describe("compareMemory — field-weighted comparison", () => {
  ...
  test("identical memories return score 1.0 and passed=true", async () => {
    ...
    expect(result.mode).toBe("legacy-3layer") // no embedding provider
  })

  test("completely different memories return score < 0.2 and passed=false", async () => {
  ...
  test("similar content, different name → fields.name < 1.0", async () => {
    const expected: ComparableMemory = {
      name: "YOLO Project",
  ...
  test("different type → fields.type = 0", async () => {
    const expected: ComparableMemory = {
  ...
  test("same type → fields.type = 1.0", async () => {
    const expected: ComparableMemory = {
      content: "Some content here",
      type: "user",
    }
  ...
  test("missing fields handled gracefully — no crash", async () => {
  ...
  test("ComparisonResult has correct structure (score, fields, passed, mode)", async () => {
    const result = await comparator.compareMemory(
    ...
    // content is now TextComparisonResult, not number
    expect(result.fields.content).toHaveProperty("score")
    expect(result.fields.content).toHaveProperty("breakdown")
  })

  test("pass threshold at 0.8 — score >= 0.8 → passed=true", async () => {
  ...
  test("content field has highest weight in overall score", async () => {
  ...
  test("description field comparison works independently", async () => {
    const expected: ComparableMemory = {
...
describe("backward compatibility — existing SemanticComparator<T>", () => {
  ...
  test("ComparisonResult type is correctly structured (with mode field)", () => {
    const result: ComparisonResult = {
      score: 0.85,
      fields: {
        content: { score: 0.9, breakdown: { tokenJaccard: 0.85, tfidf: 0.92, bigram: 0.88 } },
        name: 0.8,
        description: 0.7,
        type: 1.0,
      },
    ...
    expect(result.passed).toBe(true)
    expect(result.fields.type).toBe(1.0)
    expect(result.fields.content.score).toBe(0.9)
    expect(result.mode).toBe("legacy-3layer")
  })
})
